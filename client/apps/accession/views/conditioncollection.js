...
var View = Marionette.CollectionView.extend({
    ...
    validParenthesis: function () {
        ...
        if (l_invalid_depth.length !== 0 || r_invalid_depth.length !== 0) {
            $error_msg.html('<span class="glyphicon glyphicon-remove"></span> ' + _t('maximum depth of sub conditions groups si 3'));
            $error_msg.show();
        ...
        if (l_stack.length === 0) {
            if (r_stack.length === 0) {
                ...
                $error_msg.html('<span class="glyphicon glyphicon-remove"></span> ' + _t('invalid conditions groups'));
                $error_msg.show();
                $search_btn.prop('disabled', true);
                $save_btn.prop('disabled', true);
                return false;
