...
# class DescriptorFormatTypeDescriptorMetaModel(DescriptorFormatType):
#     """
#     Specialisation for a descriptor meta-model reference.
#     """
#
#     def __init__(self):
#         super().__init__()
#
#         self.name = "descriptor-meta-model"
#         self.group = DescriptorFormatTypeGroupReference()
#         self.verbose_name = _("Descriptor meta-model")
#         self.value_is_code = True
#         self.data = "INTEGER"
#
#     def validate(self, descriptor_type_format, value, descriptor_model_type):
...
#             return _("The descriptor value must be an integer")
#
#         # check if the descriptor meta-model exists for the model
#         try:
#             app_label, model = descriptor_type_format['model'].split('.')
#             content_type = get_object_or_404(ContentType, app_label=app_label, model=model)
#             DescriptorMetaModel.objects.get(id=value, target=content_type.id)
#         except ObjectDoesNotExist:
#             return _("The descriptor value must refers to an existing descriptor meta-model")
#
#         return None
#
#     def check(self, descriptor_type_format):
...
#         items = {}
#
#         # search for the descriptor meta-models
#         try:
#             app_label, model = descriptor_type_format['model'].split('.')
#             content_type = get_object_or_404(ContentType, app_label=app_label, model=model)
#             dmms = DescriptorMetaModel.objects.filter(id__in=values, target=content_type.id)
#         except ObjectDoesNotExist:
#             return _("The descriptor doesn't refers to a valid descriptors meta-models")
#
#         lang = translation.get_language()
#
#         for dmm in dmms.values_list('id', 'name', 'label'):
#             data = json.loads(dmm[2])
#             items[dmm[0]] = data.get(lang, dmm[1])
#
#         return {
#             'cacheable': True,
#             'items': items
